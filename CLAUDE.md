...
- `src/components/` - UI components (HoldingsTable, ValuationTable, HoldingsByType, TotalValue, MoversStrip, UpcomingEvents, NewsSection/NewsTicker, AIResearchSection, etc.)
...
- `api/holdings-history.ts` - `GET /api/holdings-history?id=…` returns the portfolio's holdings edit log (owner/full-share-link auth required)
...
- `upcoming_events` table: id, event_type (`macro`/`earnings`), event_date, event_time, title, detail, importance (**vestigial** — a constant written by `save-events.ts`; nothing reads it), tickers/holders (JSONB), holder_count, source (JSONB), position — the landing-page Upcoming Events feed (one ranked global list, replaced wholesale by `scripts/generate-events.sh`, served by `api/events.ts`)

- `holdings_history` table: portfolio_id, ticker, name, shares, prev_shares, is_static, static_value, instrument_type, cost_basis, change_type (`added`/`updated`/`removed`), recorded_at — append-only holdings edit log written by `recordHoldingsHistory` on portfolio create/edit, served by `api/holdings-history.ts`
...
- **P/E valuation tab**: the portfolio detail page's tab row is Holdings / Alloc % / P/E / Research (md+, only when a report exists) / News / History (only when the portfolio has recorded history — see below). The "P/E" tab (`src/components/ValuationTable.tsx`) lists each market-priced holding's trailing P/E, forward P/E (ongoing FY), and forward P/E next FY side by side, default-sorted by next-FY forward P/E ascending (cheapest first — that's the tab's point; all column sorts default asc, nulls at the bottom). Rows with no multiple at all (funds, static assets, loss-makers without estimates) are filtered out; if none remain the tab shows an empty-state message. A footnote legend below the table explains each column's EPS basis and bias (reported quarters can carry one-time items; next-FY estimates are cleaner but less certain) — the same distinctions as the header tooltips, made visible for touch users. Trailing P/E is served directly by companiesmarketcap.org as `peRatio` (cached in `fundamentals_cache.pe_ratio`; it's a ratio, so share-class aliasing copies it unscaled, and `computeHoldings` nulls non-positive values) — unlike the forward P/Es, which are computed from forward EPS ÷ live price. the "Top movers" pill on the landing page — the tickers swinging most today, ranked by √breadth × |move| (breadth = holder count, square-rooted so popularity has diminishing returns and the move size carries more weight) across publicly-visible (`public`/`allocation_public`) portfolios. The ranking is `computeMarketMovers` in `api/portfolios.ts`, returned as `{ regular, extended }` (one list per price basis; `LandingPage` picks one via the Extended Hours toggle, so the *ranking* switches with the basis). **Gotcha:** the collapsed display shows `DISPLAY_COUNT` (3) rows in `MoversStrip.tsx`, which must stay in sync with `MOVER_MIN_COUNT` (3) in `api/portfolios.ts`; expanding ("N more") reveals the rest up to `MOVER_MAX_COUNT` (10) — the server caps the returned list there, so the expanded pill (and the "N more" count, derived from list length) never exceeds 10 in aggregate. Each mover row carries an "i" button (shown only when the name has data) that opens a fundamentals popover — revenue, earnings, forward P/E (both the ongoing-FY figure and next-FY `forwardPENext`, computed in `snapshot.ts` from companiesmarketcap.org's `forwardEPS`/`forwardEPSNext` ÷ current price, cached in `fundamentals_cache.forward_eps`/`forward_eps_next`; that site dedupes share classes to one canonical symbol, so `FUNDAMENTALS_SHARE_CLASS_ALIASES` in `snapshot.ts` maps GOOG→GOOGL — with per-share fields scaled by the group's share ratio — else those tickers never refresh; **the site's canonical class can flip**: Berkshire was served as BRK-A (alias BRK-B→BRK-A, 1:1500 ratio) until ~Jul 2026 when the site switched to serving BRK-B directly, so that alias was removed — if a ticker's fundamentals fossilize, probe the API for which class it serves now), op margin, 3Y growth, % to 52wk high — the same figures and look as the holdings-table popover on the portfolio detail page. Those figures ride along on each mover as `MoverFundamentals`, captured from the canonical share-class holding in `computeMarketMovers` and **duplicated verbatim** as an interface in both `api/portfolios.ts` and `MoversStrip.tsx` (separate build targets — change one, change both). Layout/design rationale and rejected alternatives live in the component's code comments, not here.
- **Holdings history**: append-only edit log per portfolio. `recordHoldingsHistory` (`api/_lib/db.ts`) diffs prev-vs-next holdings on every create/edit in `api/portfolios.ts` (change types `added`/`updated`/`removed`, best-effort — never blocks the save) into the `holdings_history` table; `api/holdings-history.ts` serves it (owner/full-share-link only — allocation-only links and selective viewers denied); `useHoldingsHistory` + `src/components/HoldingsHistory.tsx` render the detail-page History tab. The tab is shown only when the fetch returns ≥1 entry (`hasHistory` in `App.tsx`), so no-history portfolios (and unauthorized viewers, whose 401/403 the hook maps to `[]`) never see an empty tab. No backfill — history starts at each portfolio's first post-migration edit. Edits made outside the web flow (direct-DB scripts) bypass the log.
- **Upcoming events strip**: `src/components/UpcomingEvents.tsx` renders the "Upcoming" pill directly below the movers strip on the landing page — a forward-looking feed of macro releases (CPI, FOMC, PCE, jobs, etc.) and earnings for stocks held by publicly-visible portfolios. Generated weekly (Sundays) by `scripts/generate-events.sh` into the `upcoming_events` table, served by `api/events.ts`, fetched via `useUpcomingEvents`. Mirrors the movers strip exactly: same card shell, a notepad-style folder tab on the card's top-left holding the icon + label, an inline blue `text-accent` "N more"/"less" expand link at the bottom-right of the last row (shares that row — not a separate centered toggle line), and a `DISPLAY_COUNT` (1, deliberately tight so the landing page leads with just the single most imminent event and stays uncluttered — unlike the movers strip's 3) cap; `save-events.ts` sorts the feed itself (**not** the generator — ordering is arithmetic over stored fields, so it's deterministic rather than re-derived by the model each run) and stamps the result into `position`, so the strip just slices the first N. **Ranking is `date` asc → macro before earnings within a date → `holder_count` desc → `id` asc.** There is no severity/importance concept anywhere in the feature: the generator's macro INCLUDE list is the whole editorial decision, breadth doesn't gate inclusion (a single-holder earnings report tomorrow ranks above a macro print next week), and the cap drops the **furthest-out** events, never the "least important" ones. Macro-before-earnings replaced an older importance tiebreak that sorted macro *last* on a shared date — macro rows carry `holder_count` 0, so breadth-desc pushed the FOMC decision below Meta and Microsoft. **Known wrinkle:** two macro events on the same date now tie down to `id` alphabetical (GDP sorts above PCE), which only shows if that date is the nearest one; fixing it would mean re-introducing a priority ranking, deliberately not done. The UI renders each row as a `date | emoji | title` statement. The date is relative for near-term events (`formatEventDate` in `src/utils/formatters.ts` returns `Today`/`1 day`/`2 days`/`3 days` for events ≤ 3 days out, calendar-day based — kept terse on purpose since these set the auto-sized date column's width) and falls back to the absolute date (`Jun 24`) beyond that window; all dates render in one color (deliberately not the accent blue, which reads as a link next to the blue `N more`/`less` toggle). Then a single decorative category emoji (`eventEmoji` in `UpcomingEvents.tsx` — 🏦 Fed/rates, 📈 inflation, 💼 jobs, 📊 growth, 🛍️ retail, 🏭 manufacturing, 💰 earnings; derived from the event, not stored) then a title, with no severity/impact dot. **Macro** titles are the stored self-contained name (the `events-prompt.md` title rule enforces plain-language titles **kept ≤ 32 chars so each renders on a single line** — the title span uses `truncate` (one-line clip, never a second line) and `save-events.ts` warns on any title over the limit). **Earnings** titles are rewritten at display time to use the **ticker**, not the stored company name (`earningsDisplayTitle` in `UpcomingEvents.tsx`: takes `tickers[0]` + the `Q#…` tail of the stored title, dropping any redundant year — so "Goldman Sachs Q2 earnings" renders "GS Q2 earnings"). This keeps the row short enough to fit the inline holder suffix on one mobile line and matches the ticker-keyed movers strip above; the stored title still names the company (and is the fallback if there's no ticker). **Earnings rows** also carry a muted "held by AV, VD" suffix appended inline after the title (`holderLabel` in `UpcomingEvents.tsx`) — the portfolio handles that own the name, uppercased, same handles as the movers strip / Users list. It names up to `HOLDER_NAME_CAP` (3) handles, then collapses to "held by N holders"; it's deliberately appended *inside* the title cell rather than given its own grid column (a meta column would reserve its widest cell's width across every row, including holder-less macro rows). Macro rows render title-only. Self-hides when the feed is empty. **Shared-tab gotcha:** the folder tab uses a fixed width (`w-36`) duplicated verbatim across all three stacked landing-page tabs (`MoversStrip`, `UpcomingEvents`, and the Users panel in `LandingPage.tsx`) so they line up at a constant width — change one, change all three. **First-load skeleton:** both `MoversStrip` and `UpcomingEvents` collapse to `null` only once their query has *settled empty*; while the first load is still in flight (React Query `isLoading`) they render a skeleton card that reserves the strip's space, so the stack doesn't reflow/pop-in above the always-rendered Users card once data lands. `MoversStrip` is presentational, so it takes `isLoading` as a prop (from the portfolios query in `LandingPage`); `UpcomingEvents` reads its own `isLoading` from `useUpcomingEvents`.
...
- **Analytics events:**
  ...
  - `share_link_id` attributes a view to the share link it arrived through (`/:portfolioId?share=<token>`). Threaded from `App.tsx` → `useViewAnalytics` (4th arg) → `log-view`. Powers the **Shared Link Access** panel on the dashboard (`computeShareLinkAccess` in `api/_lib/db.ts` → `shareLinkAccess` field), which groups each portfolio's share links with **all-time** attributed-view stats (views, unique IPs, last access, and a per-link location breakdown). Each link row shows its most-recent access location inline and expands (chevron drill-down, mirroring the Visitor Locations panel) to list every location the link's views came from (`ShareLinkAccessEntry.locations` — `display`/`count`/`lastSeenAt`, most-recent first, same `formatLocationLong` shape as the viewer-locations rows). Unlike the rest of the dashboard this is *not* windowed by `days` and *not* affected by the `excludeViewers`/"Include AV" toggle (share-link views are anonymous). Only currently-**active** links are shown — expired and revoked links are hidden (there's no status column, since every row is active). No backfill — only views logged after this shipped carry `share_link_id`.
...
- **Wrapper:** `scripts/refresh-snapshots.sh` (`flock`s a lockfile, self-syncs via `git pull --ff-only origin main` — same pattern as the news/events crons, but every tick — sources `.env.local`, logs to `scripts/refresh-snapshots.log`)

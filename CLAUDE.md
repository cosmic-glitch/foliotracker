...
- **Movers strip**: The landing page renders `src/components/MoversStrip.tsx` (rounded pill directly above the Users card, matching its width; entries wrap to extra rows when narrow — no horizontal scroll) showing the most-held tickers swinging the most today. Criteria live in `computeMarketMovers` in `api/portfolios.ts`: single-name stocks with |day move| ≥ 2% and ETFs with |day move| ≥ 1.5% (mutual funds excluded — they price once a day), held in ≥3 portfolios, GOOG/GOOGL merged, sorted by breadth × |move|. Only portfolios with publicly visible tickers (`public` or `allocation_public`) contribute. Empty array on quiet days ⇒ strip hidden — don't add filler content to it.
- **Analytics events:**

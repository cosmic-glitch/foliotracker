...
- **P/E tab**: trailing P/E arrives ready-made (`peRatio`, non-positive → null); forward P/Es are price ÷ forward EPS computed in `snapshot.ts`. Default sort next-FY forward P/E asc, nulls last
...
- **Holdings history**: `recordHoldingsHistory` diffs holdings on create/edit — best-effort, never blocks the save. Owner or full share link only; 401/403 → `[]` → the History tab hides. Direct-DB edits bypass the log

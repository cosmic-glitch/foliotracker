...
- **Movers strip**: The landing page renders `src/components/MoversStrip.tsx` (rounded pill directly above the Users card, spanning its width) showing the tickers swinging the most today, weighted by how widely they're held. Layout is a left rail — the label **Top movers** under the lucide-react `Flame` icon (a single-color, thin-stroke flame in `text-amber-500`: monochromatic, no second shade, and identical across platforms — the original look the user preferred over the native 🔥 emoji and a two-tone SVG, both kept in git history) — beside `DISPLAY_COUNT` (4) rows by default, one mover per row (expandable to the full qualified set — see Expand/collapse below). The rail is anchored to the pill's left edge (a label, echoing the left-aligned Users heading below) and the movers are **left-aligned** right beside it (`flex-1` + `justify-start`), shifted hard left to use the empty space an earlier centered (`justify-center`) layout wasted between the rail and the data. Each row is three TIGHTLY grouped columns (`auto auto auto`: ticker | day move | held-by) so a stock sits right next to its own holders — an earlier `auto auto 1fr` variant that anchored the count to the pill's far right opened an awkward gap between a stock and its "held by" text. **Held-by column — names, else count:** each row prefers naming the holders outright — `held by AB, CD` (the handles are `portfolio.id.toUpperCase()`, the same identities the Users list shows, in creation order) — and falls back to a count — `held by 3 users` (pluralized; `held by 1 user`) — only when the full name list wouldn't fit that row. The decision is per-row and width-driven: a layout effect measures the names string (canvas `measureText`) against the holders column's real available width (container width minus the ticker+move columns, which left-alignment makes a clean container-left→cell-left span), runs before paint so there's no flash, and re-runs on container resize. There are **no parentheses** around "held by" — the smaller, muted font already sets it apart, and dropping them buys width for names. Stacking the flame *above* the label keeps the rail narrow enough that one row fits even a 360px phone; type is a notch smaller on mobile than desktop (and the gap tightens) to hold that fit. Two tickers per row never fit, so we don't try. **Expand/collapse:** the pill shows `DISPLAY_COUNT` rows by default; when the server returns more (every extra row is a qualified mover — backfill only pads *up to* the floor, never past it), a small **`+N` chevron** toggle tucked *under the "Top movers" label in the rail* expands the pill in place to the full qualified set and collapses it back. Any viewer can toggle it; it's frontend-only — the API already returns the whole qualified list (`rankBy` has no upper cap), the 4-row cap was always a display-side slice in `MoversStrip`. The toggle deliberately lives in the rail's spare vertical room (the rail is shorter than the ≥4-row movers block) and stays narrower than the label, so it costs the pill **neither an extra row of height nor any horizontal width** — it never touches the movers' own rows, which the single-row mobile fit can't spare. (A full-width toggle row beneath the strip was the first cut, reverted for grabbing exactly that scarce vertical space.) Expanding adds rows that each re-run the names-vs-count fit measurement (the layout effect re-measures on `expanded` change). The explicit `Top movers` label and "held by …" wording replaced an icon-only pill that left users guessing what the flame and bare counts meant. Criteria live in `computeMarketMovers` in `api/portfolios.ts`: every live stock/ETF held in ≥1 portfolio (GOOG/GOOGL merged) is a candidate — there is **no** minimum-holders floor (an earlier rule excluded names held by fewer than 3 portfolios; removed so a big mover one person holds can still surface). Candidates are ranked by breadth × |move|, the "noteworthiness" product, so a large move held by one user (1 × 9%) can outrank a small move held by many (3 × 2%), while a calm name surfaces only when many hold it. Single-name stocks with |day move| ≥ 2% and ETFs with |day move| ≥ 1.5% (mutual funds excluded — they price once a day) are the "qualified" movers that lead. To keep the rows from coming up short, the function always returns at least `MOVER_MIN_COUNT` (4, kept in sync with `DISPLAY_COUNT`): on busy days it returns every qualifier; on quiet days it backfills the remaining slots with the next-highest-ranked names by the same product (genuine movers always lead). So the strip is effectively always visible — it only hides (empty array) in the degenerate case of fewer than 4 publicly-visible held stock/ETF names. Each `MarketMover` carries `holders: string[]` (the contributing portfolio ids, in creation order; `holders.length` is the count) — and since only `public`/`allocation_public` portfolios contribute, naming them leaks nothing not already on their public pages. **Extended-hours awareness:** `computeMarketMovers` returns `{ regular, extended }` — two independently-ranked lists. The `extended` move is the snapshot's `dayChangePercent` (current price carries the latest pre/post-market print); the `regular` move is recomputed from `regularMarketPrice` vs the same previous close (mirroring `usePortfolioData`'s regular-hours recompute). `LandingPage` shows `extended` or `regular` per the Extended Hours toggle (default off ⇒ regular), so the strip stays consistent with the holdings table and totals; since rows are ordered by |move|, the *ranking* (not just the shown %) switches with the basis. Only portfolios with publicly visible tickers (`public` or `allocation_public`) contribute.
- **Analytics events:**

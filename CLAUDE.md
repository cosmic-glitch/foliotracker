...
- **Movers strip**: The landing page renders `src/components/MoversStrip.tsx` (rounded pill directly above the Users card, matching its width) showing the most-held tickers swinging the most today. Two responsive layouts, both CSS grids with three aligned tracks per entry-column (ticker | % | held-by) flowing row-major so the tickers, percentages, and held-by counts each line up within a column: below `lg` it's two entry-columns capped at 4 (2×2); at `lg+` it's three entry-columns capped at 6 (3×2). The breakpoint is `lg` (not `md`) because the three-column grid (~480px) only fits once `max-w-4xl` caps the container and the left column gives the strip ~518px — at md the left column is too narrow, so the two-column grid is used there too. Both caps slice the same ranked list, so a truncated tail drops the least significant names. Criteria live in `computeMarketMovers` in `api/portfolios.ts`: among names held in ≥3 portfolios (GOOG/GOOGL merged), single-name stocks with |day move| ≥ 2% and ETFs with |day move| ≥ 1.5% (mutual funds excluded — they price once a day) are the "qualified" movers, ranked by breadth × |move|. To keep the pill's grid from coming up short, the function always returns at least `MOVER_MIN_COUNT` (4): on busy days it returns every qualifier; on quiet days it backfills the remaining slots with the next-highest-ranked held-by-≥3 names (genuine movers always lead). So the strip is effectively always visible — it only hides (empty array) in the degenerate case of fewer than 4 publicly-visible held-by-≥3 names. Only portfolios with publicly visible tickers (`public` or `allocation_public`) contribute.
- **Analytics events:**

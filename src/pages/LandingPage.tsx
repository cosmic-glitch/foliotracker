  ...
  return (
    ...
    >
      {/* Handle + medal — the name leads each row (left-aligned, no preceding
          ...
          it. The container is `w-22` (88px) and shrink-0 so it occupies a
          ...
          `whitespace-nowrap` but NOT clipped (no `truncate`/overflow-hidden): a
          handle too wide for the 88px box renders in full and simply overflows to
          the right, spilling into the `justify-between` gap rather than clipping
          to an ellipsis ("BAXTER" no longer becomes "BAX…"). Trade-off: a very
          long handle can run under/past the medal and toward the dollar column,
          since the overflow paints outside the box. The name is text-base (a
          touch bigger than the move line) and leading-tight; the medal is
          shrink-0. items-center pairs name and badge. */}
      <span className="flex w-22 shrink-0 items-center gap-1.5">
        <span className="min-w-0 whitespace-nowrap text-base font-semibold leading-tight text-text-primary">
